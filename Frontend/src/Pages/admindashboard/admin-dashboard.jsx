...
//     setSelectedIds((prev) => {
//       const next = new Set(prev);
//       pagedRequests.forEach((r) => next.add(r._id || r.id));
//       return next;
//     });
//   }, [pagedRequests]);
...
//     setRequests((prev) => prev.map((r) => (ids.includes(r._id || r.id) ? { ...r, status } : r)));
//     setSelectedIds((prev) => {
//       const next = new Set(prev);
...
//       setRequests((prev) => prev.map((r) => (ids.includes(r._id || r.id) ? { ...r, status: "pending" } : r)));
//     }
//   }, []);
...
//                     <button onClick={() => handleBulkAction("approved")} className="flex items-center gap-1 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
...
//                   {activeItem.type === 'request' ? (
//                     <>
//                       <button onClick={() => { handleSingleAction(activeItem.data._id || activeItem.data.id, 'rejected'); setActiveItem(null); }} className="px-5 py-2.5 rounded-lg bg-rose-50 text-rose-700 hover:bg-rose-100 font-semibold text-sm transition-colors border border-rose-100">
//                         Reject
//                       </button>
//                       <button onClick={() => { handleSingleAction(activeItem.data._id || activeItem.data.id, 'approved'); setActiveItem(null); }} className="px-5 py-2.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 font-semibold text-sm shadow-md shadow-indigo-500/20 transition-all">
//                         Accept Request
//                       </button>
//                     </>
//                   ) : (
...
// import { useNavigate } from "react-router-dom";
// import Sidebar from "./admin-components/sidebar.jsx";
// import JobsApproval from "./admin-components/JobsApproval.jsx";
// export default function AdminDashboard() {
//   const navigate = useNavigate();

//   // --- Router guard (Auth Check) ---
//   useEffect(() => {
//     try {
//       const raw = localStorage.getItem('admin_token');
//       if (!raw) {
...
//         {/* Main Content */}
//         <main className="animate-enter delay-300 flex-1 h-full overflow-hidden">
//           {view === "jobs" && <JobsApproval />}
//         </main>
//       </div>
//     </div>
//   );
...
// import JobsApproval from "./JobsApproval";
...
// src/Pages/admindashboard/admin-dashboard.jsx

import React, { useState } from "react";

// ✅ FIXED IMPORT PATHS
import Sidebar from "./admin-components/Sidebar";
import JobsApproval from "./admin-components/JobsApproval";
import AdminEventManagement from "./admin-components/AdminEventManagement";
