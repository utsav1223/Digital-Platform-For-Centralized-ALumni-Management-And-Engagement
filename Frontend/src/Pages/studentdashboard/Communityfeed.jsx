// src/Pages/studentdashboard/Communityfeed.jsx
                  ...
                  <div className={`flex flex-col sm:flex-row sm:items-center justify-between mt-2 pt-2 border-t border-slate-50 transition-all duration-300 origin-top ${isPosting ? 'opacity-100 max-h-[200px]' : 'opacity-0 max-h-0 overflow-hidden'}`}>
                    ...
                    <div className="flex items-center gap-3 justify-end">
                      ...
                      <button 
                        ...
                        className="px-4 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-all shadow-sm flex items-center gap-2"
                      >
        ...
        {post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4">
            {post.tags.map(tag => (
              <span 
                key={tag} 
                className="text-xs font-medium text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full hover:bg-indigo-100 cursor-pointer transition-colors border border-indigo-100"
              >
                {tag}
              </span>
            ))}
...
const IconButton = ({ icon, label }) => (
  <button 
    type="button" 
    className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-indigo-600 transition-colors tooltip" 
    title={label}
  >
    {icon}
  </button>
...
const ActionButton = ({ icon, label, active, onClick, colorClass = "text-slate-500" }) => (
  <button 
    onClick={onClick} 
    className={`group flex items-center gap-1.5 sm:gap-2 py-1.5 px-2 sm:px-3 -ml-2 rounded-lg transition-all hover:bg-slate-50 active:scale-95 ${
